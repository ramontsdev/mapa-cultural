...
import type { MeuEventoRecord } from "@/lib/types";
import { CalendarDays, ChevronRight, Pencil, Plus, Search } from "lucide-react";
...
export default function MeusEventosPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(
        (r) =>
          r.evento.nome.toLowerCase().includes(q) ||
          r.evento.descricao.toLowerCase().includes(q)
      );
    }
...
function ListaMeusEventos({
  items,
  emptyLabel,
}: {
  items: MeuEventoRecord[];
  emptyLabel: string;
}) {
  if (items.length === 0) {
    return (
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {new Date(r.evento.dataInicio).toLocaleDateString("pt-BR")}
                </p>
