...
import type { MeuProjetoRecord } from "@/lib/types";
import { ChevronRight, FolderKanban, Pencil, Plus, Search } from "lucide-react";
    ...
    if (search.trim()) {
      ...
      list = list.filter(
        (r) =>
          ...
          r.projeto.descricao.toLowerCase().includes(q)
      );
    }
        ...
        <Card key={r.id}>
          <CardContent className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {TIPO_PROJETO_LABELS[r.projeto.tipo]}
                </p>
            ...
            <div className="flex flex-wrap gap-2 sm:justify-end">
              <Button variant="outline" asChild>
                <Link href={`/projetos/${r.id}`}>
                  Acessar
                  <ChevronRight className="ml-1 h-4 w-4" />
                </Link>
              </Button>
              <Button asChild>
                <Link href={`/projetos/${r.id}/editar`}>
