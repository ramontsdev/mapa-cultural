...
export function CreateAgenteDialog({
  open,
  onOpenChange,
  onCriadoRascunho,
  onCriadoPublicado,
}: Props) {
  const form = useForm<CriarAgenteRapidoFormData>({
    resolver: zodResolver(criarAgenteRapidoSchema),
    defaultValues: {
      nome: "",
  ...
  const bioLen = form.watch("biografia")?.length ?? 0;
  ...
  const submit = (status: "draft" | "published") => {
    void form.handleSubmit((data) => {
      const idEmail =
        data.email?.trim() ||
        `agente-${Date.now()}@cadastro-local.mapa-cultural.invalid`;
      const record = buildNewMeuAgenteRecord(
        {
          nome: data.nome,
          email: idEmail,
      ...
      <DialogContent className="flex max-h-[100dvh] w-full max-w-[calc(100%-2rem)] flex-col gap-0 overflow-hidden rounded-none p-0 sm:max-h-[90vh] sm:max-w-lg sm:rounded-lg">
        <div className="overflow-y-auto p-6 pb-4">
          <DialogHeader>
            ...
            <DialogDescription>
              Perfil básico no mapa. Complete dados depois na edição.
            </DialogDescription>
          </DialogHeader>
